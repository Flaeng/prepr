    ...
    internal static void WriteFolderFilesRule(ScanResult result, string rootPath, TextWriter writer, ReportOptions options)
    {
        ...
        else
        {
            ...
            foreach (var v in violations)
            {
                ...
                var prompt = WebUtility.HtmlEncode(v.GetPrompt(relativePath));
                writer.WriteLine($"""<tr><td class="px-6 py-4 font-mono text-xs text-primary">{WebUtility.HtmlEncode(relativePath)}</td><td class="px-6 py-4 text-right font-bold text-tertiary">{v.FileCount}</td><td class="px-6 py-4 text-right text-on-surface-variant">{v.Limit}</td>{SeverityCell(v.Severity)}<td class="px-6 py-4 text-right whitespace-nowrap"><button data-prompt="{prompt}" onclick="showPromptModal(this)" class="px-2 py-1 rounded text-[10px] font-bold bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-colors mr-1" style="cursor:pointer">Show prompt</button><button data-prompt="{prompt}" onclick="copyPrompt(this)" class="px-2 py-1 rounded text-[10px] font-bold bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-colors" style="cursor:pointer">Copy prompt</button></td></tr>""");
            }
