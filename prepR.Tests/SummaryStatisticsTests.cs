...
public class SummaryStatisticsTests
{
    ...
    public void Compute_SingleBlock_CorrectStats()
    {
        ...
        var result = new ScanResult([block], 3, 100, new Dictionary<string, int>(), new Dictionary<string, NestingDepthInfo>(), new Dictionary<string, IReadOnlyList<EarlyReturnViolation>>(), new Dictionary<string, int>(), new Dictionary<string, IReadOnlyList<MagicNumberViolation>>(), new Dictionary<string, IReadOnlyList<MagicStringViolation>>());
        var stats = SummaryStatistics.Compute(result);
