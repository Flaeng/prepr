...
public class FilePairGroupTests
{
    ...
    public void ComputeFilePairs_TwoFilesOneBlock_OnePair()
    {
        ...
        var result = new ScanResult([block], 2, 100, new Dictionary<string, int>(), new Dictionary<string, (int MaxDepth, IReadOnlyList<(int LineNumber, int Depth)> LineDepths)>(), new Dictionary<string, IReadOnlyList<EarlyReturnViolation>>());
        var pairs = FilePairGroup.ComputeFilePairs(result);
    ...
    public void ComputeFilePairs_TwoBlocksSameFiles_OnePairTwoBlocks()
    {
        ...
        var result = new ScanResult([block1, block2], 2, 100, new Dictionary<string, int>(), new Dictionary<string, (int MaxDepth, IReadOnlyList<(int LineNumber, int Depth)> LineDepths)>(), new Dictionary<string, IReadOnlyList<EarlyReturnViolation>>());
        var pairs = FilePairGroup.ComputeFilePairs(result);
    ...
    public void ComputeFilePairs_SortedBySharedBlockCountDescending()
    {
        ...
        var result = new ScanResult([block1, block2, block3], 4, 200, new Dictionary<string, int>(), new Dictionary<string, (int MaxDepth, IReadOnlyList<(int LineNumber, int Depth)> LineDepths)>(), new Dictionary<string, IReadOnlyList<EarlyReturnViolation>>());
        var pairs = FilePairGroup.ComputeFilePairs(result);
